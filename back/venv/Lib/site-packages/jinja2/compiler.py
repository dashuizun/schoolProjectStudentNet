...
def generate(
    ...
    if not isinstance(node, nodes.Template):
        raise TypeError("Can't compile non static nodes")
...
class CodeGenerator(NodeVisitor):
    def __init__(
        ...
        # can safely assume that we're a child static and do some
        # more optimizations.
    ...
    def visit_Template(self, node, frame=None):
        ...
        for name, block in iteritems(self.blocks):
            ...
            # toplevel static.  This would cause a variety of
    ...
    def visit_Block(self, node, frame):
        """Call a block and register it for the static."""
        level = 0
        if frame.toplevel:
            # if we know that we are a child static, there is no need to
            # check if we are one
    ...
    def visit_Extends(self, node, frame):
        ...
        # the static before this one.
        if self.extends_so_far > 0:

            # if we have a known extends we just add a static runtime
            ...
            # as we know that the static execution will end here.
            if self.has_known_extends:
                raise CompilerExit()
            else:
                self.outdent()
    ...
    def visit_Include(self, node, frame):
        ...
        self.writeline("static = environment.%s(" % func_name, node)
        self.visit(node.template, frame)
        self.write(", %r)" % self.name)
        if node.ignore_missing:
            self.outdent()
        ...
        if node.with_context:
            loop = self.environment.is_async and "async for" or "for"
            self.writeline(
                "%s event in static.root_render_func("
                "static.new_context(context.get_all(), True, "
                "%s)):" % (loop, self.dump_local_context(frame))
            )
        elif self.environment.is_async:
            self.writeline(
                "for event in (await "
                "static._get_default_module_async())"
                "._body_stream:"
            )
        else:
            if supports_yield_from:
                self.writeline("yield from static._get_default_module()._body_stream")
                skip_event_yield = True
            else:
                self.writeline(
                    "for event in static._get_default_module()._body_stream:"
                )
        ...
        for name in node.names:
            ...
            self.writeline(
                ...
                % (
                    ...
                    "the static %%r (imported on %s) does "
    ...
    def visit_For(self, node, frame):
        ...
        # make sure the loop variable is a special one and raise a static
    ...
    def _output_const_repr(self, group):
        ...
        child nodes, produce a string to write to the static module
        source.
        """
        ...
        for child in node.nodes:
            try:
                if not (
                    ...
                    finalize.const
                    # Unless it's basic static data that won't be
                    # finalized anyway.
